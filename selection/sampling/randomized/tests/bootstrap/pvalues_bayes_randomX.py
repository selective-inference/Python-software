...
def pval(vec_state, full_projection,
    ...
    if set(nonzero).issubset(active_set):
        for j, idx in enumerate(active_set):
            ...
            T_minus_j = np.dot(linear_part, beta_unpenalized)
            ...
            def full_gradient(vec_state, fixed_part=fixed_part, obs_residuals=obs_residuals,
                ...
                omega = - fixed_part - XXc * beta_bar_boot[j]+ np.dot(hessian, beta_full) + opt_vec
            ...
            beta_bars = [np.dot(XEpinv, np.diag(obs_residuals)).dot(alpha_samples[i,:]) for i in range(len(samples))]
