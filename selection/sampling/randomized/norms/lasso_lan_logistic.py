...
class selective_l1norm_lan_logistic(rr.l1norm, selective_penalty):
    ...
    def step_variables(self, state, randomization, logpdf, gradient, SigmaInv, hessian, P, R):
        """
        ...
        grad_T = np.dot(hessian[:, active].T, sign_vec)
        ...
        #data_proposal = np.concatenate((_proposal, data[nactive:]), axis=0)
