...
def MLE_family(constraint, 
    ...
    r"""
    Sample from one parameter family
    after finding MLE tilt for observed data.
    ...
    MLE_opts : dict
        Args passed to `one_parameter_MLE`.

    Returns
    -------

    family : `selection.distributions.discrete_family`
        Discrete exponential family tilted so that
        observed value is roughly the MLE of
        the mean parameter.
        
    """

    con_cp = copy(constraint)
    MLE = one_parameter_MLE(constraint,
                            initial,
                            direction_of_interest,
                            **MLE_opts)
    ...
    tilt = MLE * direction_of_interest
    con_cp.mean += np.dot(con_cp.covariance, tilt)

    _, Z, W, _ = gibbs_test(con_cp, 
                            initial,
                            direction_of_interest,
                            UMPU=False,
                            sigma_known=True,
    ...
    null_statistics = np.dot(Z, direction_of_interest)
    observed = (initial * direction_of_interest).sum()
    logW = -np.dot(Z, tilt)
    logW -= logW.max() - 5
