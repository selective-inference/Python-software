...
# sample tsls statistic via linear decomposition method, asymptotic method

# here is the new version which is the alternative way in the manuscript
...
def solve(Z, D, Y, 
    ...
    # Compute u_{-E} = u_{nE}, 
    # This is from the KKT condition -Z_{-E}^T (Y - Z_{E} alpha_E - D beta) + \lambda u_E - w_{-E} = 0
...
class MH_tsls(object):
    ...
    def __init__(self, 
        ...
        # compute observed tsls statistic value
        self.T_obs = D.dot(self.P_diff).dot(Y - D * self.b0) / np.sqrt(self.Sigmab0[0,0]*D.dot(self.P_diff).dot(D))

        # compute K constants
        # K1 = \Sigma_{S,T}
        # note we are trying the alternative selective density now
        DPD = D.dot(self.P_diff).dot(D)

        self.K1 = np.hstack([Z.T.dot(self.P_diff).dot(D).reshape(1,-1), np.atleast_2d(D.dot(self.P_diff).dot(D))])
        #self.K1 *= (self.Sigmab0[0,0]+self.b0*self.Sigmab0[0,1]) / np.sqrt(self.Sigmab0[0,0]*D.dot(P_diff).dot(D))
        self.K1 *= np.sqrt(self.Sigmab0[0,0]/DPD)
        ...
        self.K3 = np.vstack([np.identity(p),D.dot(Z).dot(ZTZ_Inv)]).dot(Z.T.dot(Y)).reshape(1,-1) - self.K1 * self.T_obs

        # these will be used in tCI
        self.K4 = np.vstack([np.identity(p),D.dot(Z).dot(ZTZ_Inv)]).dot(Z.T.dot(Y)).reshape(1,-1)
        self.K32 = D.dot(self.P_diff).dot(Y) / np.sqrt(self.Sigmab0[0,0] * DPD)
        self.K4 -= self.K1 * self.K32
        self.K33 = np.sqrt(DPD/self.Sigmab0[0,0])
        self.K5 = self.K1 * self.K33
    ...
    def log_pdf_data(self, state):
        # here tsls uses standard normal distribution
    ...
    def step_data(self):
        ...
        data_proposal = data + self.data_stepsize * np.random.standard_normal(1)
    ...
    # this will only be used in tCI
    def tilted_reconstruction_map(self, state, b0):
        T = state[self.state_Tindex]
        alpha_E = state[self.state_alpha_Eindex]
        beta = state[self.state_betaindex]
        u_nE = state[self.state_u_nEindex]        
        ...
        subgrad[:-1][self.E] = self.s_E
        subgrad[:-1][~self.E] = u_nE
        omega = - self.K1 * T + self.K2.dot(coefs) + self.lagrange * subgrad - self.K4 - self.K5 * b0
        return omega
...
class MH_sampler(MH_tsls):
    ...
    def sample(self, ndraw = 500, burnin = 100, returnAllSamples = False):
        """
        The function samples the distribution of the tsls statistic
