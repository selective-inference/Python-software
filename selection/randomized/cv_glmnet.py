...
class CV_glmnet(object):
    ...
    def choose_lambda_CVR(self,  scale1 = None, scale2=None, loss=None):
        if loss is None:
            loss = self.loss
        _, _, lam_seq, CV_err, SD = self.using_glmnet(loss)

        rv1, rv2 = np.zeros(lam_seq.shape[0]), np.zeros(lam_seq.shape[0])
        if scale1 is not None:
            randomization1 = randomization.isotropic_gaussian((lam_seq.shape[0],), scale=scale1)
            rv1 = np.asarray(randomization1._sampler(size=(1,)))
        if scale2 is not None:
            randomization2 = randomization.isotropic_gaussian((lam_seq.shape[0],), scale=scale2)
            rv2 = np.asarray(randomization2._sampler(size=(1,)))
        CVR = CV_err+rv1.flatten()+rv2.flatten()
        lam_CVR = lam_seq[np.argmin(CVR)] # lam_CVR minimizes CVR
        CV1 = CV_err+rv1.flatten()
        return  lam_CVR, SD, CVR, CV1
    ...
    def bootstrap_CVR_curve(self, scale1=None, scale2=None):
        ...
        def _bootstrap_CVerr_curve(indices):
            ...
            _, _, CVR_val, CV1_val = self.choose_lambda_CVR(scale1, scale2, loss_star)
