...
def simulate(n=200, p=100, s=5, signal=(3, 4), sigma=2, alpha=0.1, glmnet=True):
    ...
    if len(observed_list) > 0:
        ...
        while True:
            ...
            linfunc = np.linalg.pinv(X[:,observed_list])[i]
            true_target = linfunc.dot(X.dot(truth))
            print(true_target, 'target')
            if np.fabs(true_target) > 1.e-10:
...
if __name__ == "__main__":
    ...
    for i in range(500):
        ...
        csvfile = 'lasso_exact_variable_nonnull.csv'
